...
class HrDetailsScreen extends StatefulWidget {
  ...
  const HrDetailsScreen({Key? key, required this.organization}) : super(key: key);
...
class _HrDetailsScreenState extends State<HrDetailsScreen> {
  ...
  Future<void> _saveInline() async {
    ...
    if (owner != null) {
      ...
      final changed = newName != owner.name || newEmail != owner.email || newPhone != owner.phone;
      if (changed) {
        ...
        child: Container(
          ...
          child: Column(
            children: [
              ...
              Row(children: [
                Expanded(child: Text(resolved, style: const TextStyle(fontSize: 12, color: Colors.blueGrey), overflow: TextOverflow.ellipsis)),
                IconButton(icon: const Icon(Icons.copy, size: 18), onPressed: () { Clipboard.setData(ClipboardData(text: resolved)); ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Link copied'))); })
              ]),
          ...
          Expanded(
            child: Obx(() {
              ...
              final initials = org.name.isNotEmpty
                  ...
                  : 'O';
               final logoUrl =
                   (org.logo ?? '').isNotEmpty ? _resolveMediaUrl(org.logo!) : '';
               return Row(children: [
                 CircleAvatar(
                   radius: 36,
               ...
               itemBuilder: (_) {
                 ...
                 items.add(PopupMenuItem<String>(
                   ...
                   child: ListTile(
                     dense: true,
                     contentPadding: EdgeInsets.zero,
                     leading: Icon(org.isBlocked ? Icons.lock_open : Icons.block, color: org.isBlocked ? Colors.green : Colors.red),
                     title: Text(org.isBlocked ? 'Unblock Organization' : 'Block Organization'),
                   ),
                 ));
                // Owner user block/unblock action (admin)
                final owner = userController.user.value; // Block state not on User model; both actions available
                if (owner != null) {
                  items.add(const PopupMenuItem<String>(
                    value: 'block_user',
                    child: ListTile(
  ...
  Widget _buildUserInfoSection() => Container(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [ Icon(Icons.person, color: Colors.blue[600]), const SizedBox(width: 12), const Text('Owner User Information', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)) ]),
          const SizedBox(height: 16),
          Obx(() {
            if (userController.isLoading.value) return const Center(child: CircularProgressIndicator());
            if (userController.hasError.value) {
              return _errorBox('Failed to load user', userController.errorMessage.value, onRetry: () => userController.fetchUserById(widget.organization.userId));
            }
            final User? user = userController.user.value; if (user == null) return const Text('No user data', style: TextStyle(color: Colors.grey));
            return Column(children: [
              _editableInfoRow('Name', _ownerNameCtrl, user.name, Icons.person_outline, required: true, maxLen: 120),
              _editableInfoRow('Email', _ownerEmailCtrl, user.email, Icons.email_outlined, email: true),
              _editableInfoRow('Phone', _ownerPhoneCtrl, user.phone, Icons.phone_outlined, phone: true),
      ...
      child: Form(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [
            ...
            const SizedBox(width: 12),
            const Text('Organization Information', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
          ]),
          const SizedBox(height: 16),
          ...
          _displayRow('Registration No', org.registrationNumber, Icons.confirmation_number),
          const SizedBox(height: 12),
          _inlineField('Description', _descCtrl, maxLines: 3, validator: (v){
            if(!_editMode) return null; final val = v?.trim() ?? ''; if(val.length>2000) return 'Description too long'; return null; }),
          const SizedBox(height: 12),
          _sectionLabel('Contact'),
          const SizedBox(height: 8),
            _inlineField('Phone', _phoneCtrl, validator: (v){
              if(!_editMode) return null; final val = v?.trim() ?? ''; if(val.isEmpty) return null; if(!RegExp(r'^[- +0-9]{6,15}$').hasMatch(val)) return 'Invalid phone'; return null; }),
          const SizedBox(height: 12),
          _inlineField('Email', _emailCtrl, validator: (v){
            if(!_editMode) return null; final val = v?.trim() ?? ''; if(val.isEmpty) return null; if(!RegExp(r'^[^@\s]+@[^@\s]+\.[^@\s]+$').hasMatch(val)) return 'Invalid email'; return null; }),
          ...
          _inlineField('Pincode', _pincodeCtrl, validator: (v){
            if(!_editMode) return null; final val = v?.trim() ?? ''; if(val.isEmpty) return null; if(val.length < 3) return 'Too short'; if(val.length>12) return 'Too long'; return null; }),
          const SizedBox(height: 12),
  ...
  Widget _inlineField(String label, TextEditingController controller, {int maxLines = 1, String? Function(String?)? validator}) {
    ...
    if (_editMode) TextFormField(
      ...
      decoration: InputDecoration(
        ...
        enabledBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(8), borderSide: const BorderSide(color: Color(0xFFD0D0D0))),
      ...
      child: Text(controller.text.isNotEmpty ? controller.text : '—', style: TextStyle(color: controller.text.isEmpty? Colors.grey: Colors.black87)),
    )]);
  }
  ...
  Widget _readOnlyField(String label, String value) => Column(crossAxisAlignment: CrossAxisAlignment.start, children:[
    Row(children:[Icon(Icons.label_important, size: 16, color: Colors.grey[600]), const SizedBox(width:6), Text(label, style: const TextStyle(fontSize:13,fontWeight: FontWeight.w600))]),
    const SizedBox(height:6),
    Container(
      width: double.infinity,
      padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 10),
      decoration: BoxDecoration(color: Colors.grey[50], borderRadius: BorderRadius.circular(8), border: Border.all(color: Colors.grey[300]!)),
      child: Text(value.isNotEmpty ? value : '—', style: TextStyle(color: value.isEmpty? Colors.grey: Colors.black87)),
    )
  ]);
  ...
  Widget _displayRow(String label, String value, IconData icon) {
    return Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      const SizedBox(width: 8),
      Expanded(child: Text(value.isNotEmpty ? value : '—', style: TextStyle(color: value.isEmpty? Colors.grey: Colors.black87))),
    ]);
  }
  ...
  Widget _buildLocationSection(Organization org) => Container(
        ...
        decoration: _cardDecoration(),
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [ Icon(Icons.location_on, color: Colors.orange[600]), const SizedBox(width: 12), const Text('Location', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)) ]),
          const SizedBox(height: 16),
          ...
          _editableInfoRow('Pincode', _pincodeCtrl, org.pincode, Icons.local_post_office, pincode: true),
          _editableInfoRow('Latitude', _latCtrl, org.latitude, Icons.gps_not_fixed, latitude: true),
          _editableInfoRow('Longitude', _lngCtrl, org.longitude, Icons.gps_fixed, longitude: true),
        ]),
      );
  Widget _editableInfoRow(String label, TextEditingController controller, String currentValue, IconData icon, {bool required = false, bool email = false, bool phone = false, bool pincode = false, bool latitude = false, bool longitude = false, int maxLen = 255}) {
    String? validator(String? v) {
      if(!_editMode) return null; final val = v?.trim() ?? '';
      if (required && val.isEmpty) return 'Required';
      if (val.isNotEmpty && val.length > maxLen) return 'Too long';
      if (email && val.isNotEmpty && !RegExp(r'^[^@\s]+@[^@\s]+\.[^@\s]+$').hasMatch(val)) return 'Invalid email';
      if (phone && val.isNotEmpty && !RegExp(r'^[- +0-9]{6,15}$').hasMatch(val)) return 'Invalid phone';
      if (pincode && val.isNotEmpty && (val.length < 3 || val.length > 12)) return 'Invalid pincode';
      if (latitude && val.isNotEmpty) { final d = double.tryParse(val); if(d==null || d < -90 || d > 90) return 'Invalid latitude'; }
      if (longitude && val.isNotEmpty) { final d = double.tryParse(val); if(d==null || d < -180 || d > 180) return 'Invalid longitude'; }
      return null;
    }
    return Container(
      ...
      child: Row(crossAxisAlignment: CrossAxisAlignment.start, children:[
        ...
        SizedBox(width: 140, child: Text(label, style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w600))),
        const SizedBox(width: 12),
        Expanded(child: !_editMode ? Text(currentValue.isNotEmpty ? currentValue : '—', style: TextStyle(color: currentValue.isEmpty? Colors.grey: Colors.black87))
          : TextFormField(
            controller: controller,
            validator: validator,
            decoration: InputDecoration(
  ...
  Widget _buildStatsSection(Organization org) => Container(
        ...
        decoration: _cardDecoration(),
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [ Icon(Icons.insights, color: Colors.purple[600]), const SizedBox(width: 12), const Text('Statistics', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)) ]),
          const SizedBox(height: 16),
  ...
  Widget _buildDocumentsSection(Organization org) => Container(
        ...
        decoration: _cardDecoration(),
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [ Icon(Icons.picture_as_pdf, color: Colors.red[600]), const SizedBox(width: 12), const Text('Documents', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)) ]),
          const SizedBox(height: 16),
          if (org.document.isEmpty) const Text('No documents uploaded', style: TextStyle(color: Colors.grey)) else ListView.builder(
            shrinkWrap: true,
            ...
            itemBuilder: (_, i) {
              ...
              return Container(
                ...
                child: Row(children: [
                  Icon(isPdf ? Icons.picture_as_pdf : Icons.insert_drive_file, color: isPdf ? Colors.red : Colors.blue, size: 24),
                  const SizedBox(width: 12),
                  Expanded(child: Text(resolved.split('/').last, style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w500), overflow: TextOverflow.ellipsis)),
                  TextButton.icon(onPressed: () => _viewDocument('Document ${i + 1}', doc), icon: const Icon(Icons.visibility, size: 16), label: const Text('View'))
                ]),
              );
            },
          )
  ...
  Widget _buildImagesSection(Organization org) => Container(
        ...
        decoration: _cardDecoration(),
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [ Icon(Icons.image, color: Colors.teal[600]), const SizedBox(width: 12), const Text('Images', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)) ]),
          const SizedBox(height: 16),
          if (org.images.isEmpty) const Text('No images uploaded', style: TextStyle(color: Colors.grey)) else Wrap(spacing: 12, runSpacing: 12, children: org.images.map((img) {
            final resolved = _resolveMediaUrl(img);
            return GestureDetector(onTap: () => _viewDocument('Image', img), child: Container(width: 120, height: 90, decoration: BoxDecoration(borderRadius: BorderRadius.circular(8), border: Border.all(color: Colors.grey[300]!), image: DecorationImage(image: NetworkImage(resolved), fit: BoxFit.cover))));
          }).toList())
        ]),
      );
  ...
  void _handleApprove() {
    showDialog(context: context, builder: (_) => AlertDialog(title: const Text('Approve Organization'), content: Text('Approve organization ID: ${widget.organization.id}?'), actions: [
      TextButton(onPressed: () => Navigator.pop(context), child: const Text('Cancel', style: TextStyle(color: Colors.grey))),
      ElevatedButton(style: ElevatedButton.styleFrom(backgroundColor: Colors.white, foregroundColor: Colors.green, side: const BorderSide(color: Colors.green), elevation: 0), onPressed: () { Navigator.pop(context); hrController.approveOrganization(widget.organization.id); }, child: const Text('Approve')),
    ]));
  }

  void _handleReject() {
    showDialog(context: context, builder: (_) => AlertDialog(title: const Text('Reject Organization'), content: Text('Reject organization ID: ${widget.organization.id}?'), actions: [
      TextButton(onPressed: () => Navigator.pop(context), child: const Text('Cancel', style: TextStyle(color: Colors.grey))),
      ElevatedButton(style: ElevatedButton.styleFrom(backgroundColor: Colors.white, foregroundColor: Colors.red, side: const BorderSide(color: Colors.red), elevation: 0), onPressed: () { Navigator.pop(context); hrController.rejectOrganization(widget.organization.id); }, child: const Text('Reject')),
    ]));
  }
  ...
  void _handleRevokeApproval() {
    ...
    showDialog(context: context, builder: (_) => AlertDialog(
      ...
      content: Text('Revoke approval for organization "${org.name}" (ID: ${org.id})? This will move it back to Pending status.'),
      actions: [
        TextButton(onPressed: () => Navigator.pop(context), child: const Text('Cancel', style: TextStyle(color: Colors.grey))),
        ElevatedButton(
          style: ElevatedButton.styleFrom(backgroundColor: Colors.white, foregroundColor: Colors.orange, side: const BorderSide(color: Colors.orange), elevation: 0),
          onPressed: () {
  ...
  void _handleBlockOwnerUser() {
    final owner = userController.user.value;
    if (owner == null) {
      Get.snackbar('Error', 'Owner user not loaded yet', snackPosition: SnackPosition.BOTTOM, backgroundColor: Colors.red[100], colorText: Colors.red[800]);
      return;
    }
    showDialog(context: context, builder: (_) => AlertDialog(
      title: const Text('Block Owner User'),
      content: Text('Are you sure you want to block user "${owner.name}" (ID: ${owner.id})? This action may restrict account access.'),
      actions: [
        TextButton(onPressed: () => Navigator.pop(context), child: const Text('Cancel', style: TextStyle(color: Colors.grey))),
        ElevatedButton(
  ...
  Widget _blockedStatusChip() => Container(
    ...
    child: Row(mainAxisSize: MainAxisSize.min, children:[
      ...
      const SizedBox(width: 8),
      Text('Blocked', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: Colors.red[700])),
    ]),
  );
          ...
          child: ConstrainedBox(
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  Row(children:[
                    const Text('Jobs', style: TextStyle(fontSize: 20,fontWeight: FontWeight.w700)),
                    const SizedBox(width: 8),
                    Obx(() => IconButton(
                      tooltip: 'Refresh',
                      ...
                      onPressed: hrController.isLoadingJobs.value ? null : () async {
                        await hrController.fetchOrganizationJobs(org.id, force: true);
                      },
                    )),
                    const Spacer(),
                  ...
                  if (!loading && error != null) Expanded(child: Center(child: _errorBox('Jobs Error', error, onRetry: () { Navigator.of(ctx).pop(); _openJobsList(); }))),
                  if (!loading && error == null && jobs.isEmpty) const Expanded(child: Center(child: Text('No jobs found'))),
                    ...
                    Expanded(
                      child: ListView.separated(
                        ...
                        itemBuilder: (_, index) {
                          ...
                          final loc = job.city.isNotEmpty ? '${job.city}, ${job.state}' : job.state;
                          final salary = job.salary.isNotEmpty ? job.salary : '—';
                          final idText = job.id != null ? 'ID: ${job.id}' : '';
                          return ListTile(
                            leading: CircleAvatar(
                              radius: 18,
                              backgroundColor: Colors.blue[50],
                              child: const Icon(Icons.work_outline, size: 18, color: Colors.blue),
                            ),
